class PublicationsController < ApplicationController
  ...
  def index
    @distributions = @site.site_articles.includes(:category, article: :feed)
  ...
  def create
    ...
    distribution.save!
    redirect_to gastronomy_publications_path, notice: "Matéria adicionada à fila da revista."
  end
  ...
  def update
    ...
    category = @site.categories.find_by(id: publication_params[:category_id].presence)
    @distribution.assign_attributes(
      placement:,
      position: publication_params[:position].to_i.clamp(0, 999),
      category:,
    ...
    redirect_to gastronomy_publications_path, notice: "Revista atualizada. A nova organização já está disponível para a API."
  end
